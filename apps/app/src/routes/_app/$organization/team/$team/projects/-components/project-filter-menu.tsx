...
import { projectFilterOptions } from "@/config/team";
import { useProjectFilters } from "@/hooks/use-project-filter-store";
...
interface ProjectFilterMenuProps extends React.ComponentProps<
  typeof DropdownMenu
> {}
...
export function ProjectFilterMenu({
  ...
  const { addFilter, filters } = useProjectFilters(team);
  ...
  const flattenedFilterOptions = React.useMemo(() => {
    ...
    if (lowerSearch.length < 2) {
      return projectFilterOptions.map((f) => ({
    ...
    projectFilterOptions.forEach((filter) => {
            ...
            <React.Fragment>
              {flattenedFilterOptions.map((item) => {
                if (item.type === "category") {
                  const filter = projectFilterOptions.find(
                    (f) => f.id === item.id
                  )!;
